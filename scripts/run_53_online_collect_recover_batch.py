...
RUN_RECOVER = SCRIPT_DIR / "run_53_online_partial_recovery.py"


def parse_args() -> tuple[argparse.Namespace, list[str], list[str]]:
    ap = argparse.ArgumentParser(
        description="Batch online signal collection followed by immediate online key-byte recovery."
        ,
        allow_abbrev=False,
    )
    ...
    ap.add_argument(
        "--recover-python",
        default="",
        help="Optional Python interpreter for the recovery step. Defaults to the current interpreter.",
    )
...
def parse_true_rank(summary_json: Path, byte_positions: list[int]) -> dict[int, dict]:
    out: dict[int, dict] = {}
...
def main() -> None:
    ...
    require_root("[!] run_53_online_collect_recover_batch.py requires root because collection requires root.")
    ...
    outdir = (
        ...
        else timestamped_outdir_ch5("exp5_3_online_collect_recover_batch")
    )
    outdir.mkdir(parents=True, exist_ok=True)
    print_banner(f"Experiment 5.3 Online Collect+Recover Batch ({args.runs} runs)", outdir)
    ...
    for i in range(1, int(args.runs) + 1):
        ...
        collect_cmd = [sys.executable, str(RUN_COLLECT), *collect_argv]
        if "--reuse-model-json" not in collect_cmd:
            collect_cmd.extend(["--reuse-model-json", str(Path(args.model_json).resolve())])
        collect_cmd.extend(["--outdir", str(collect_out)])
        if i > 1:
        ...
        recover_cmd = [
            ...
            str(RUN_RECOVER),
            "--model-json",
            str(Path(args.model_json).resolve()),
            "--observations-csv",
        ...
        print(f"[batch-online] collect {i}/{args.runs}: {' '.join(collect_cmd)}")
        cp_collect = subprocess.run(collect_cmd, check=False)

        cp_recover_rc = -1
        parsed = {}
        if cp_collect.returncode == 0:
            print(f"[batch-online] recover {i}/{args.runs}: {' '.join(recover_cmd)}")
            cp_recover = subprocess.run(recover_cmd, check=False)
            cp_recover_rc = int(cp_recover.returncode)
            if cp_recover_rc == 0:
                parsed = parse_true_rank(recover_out / "partial_online_summary.json", byte_positions)
        ...
        if args.stop_on_fail and (cp_collect.returncode != 0 or cp_recover_rc != 0):
            raise SystemExit(
                f"[batch-online] failed: run={i} collect_rc={cp_collect.returncode} recover_rc={cp_recover_rc}"
            )
    ...
    print(f"[batch-online] done: outdir={outdir}")
